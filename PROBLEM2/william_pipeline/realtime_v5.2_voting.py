...
def process_video(video_path, model, resolution, device='cpu'):
    ...
    # Add these new variables near the other cooldown variables:
    # NEW: Add global cooldown tracking to prevent any scan (regardless of class) within cooldown period
    last_any_scan_time = 0  # frame number of last scan of ANY product
    
    # NEW: Track potential reappearances to avoid canceling scans prematurely
    reappearance_tracking = {}  # track_id -> [first_seen_frame, consecutive_frames]
    reappearance_threshold = 3  # Require at least 3 consecutive frames before canceling
        ...
        if frame_num % 1 == 0:
            ...
            for track_id in list(objects_in_scan_zone.keys()):
                ...
                if track_id in class_votes and class_votes[track_id]:
                    ...
                    # NEW: Check if any product was scanned recently (global cooldown)
                    if frame_num - last_any_scan_time < cooldown_frames:
                        print(f"⏸️ [{timestamp}] Global cooldown active, waiting... (ID: {track_id})")
                    ...
                    # Check if this class has been scanned recently (class-specific cooldown)
                    if most_voted_cls not in last_class_scan_time or \
                       frame_num - last_class_scan_time[most_voted_cls] >= cooldown_frames:
                        ...
                        if max_x_positions.get(track_id, 0) >= scan_threshold_x and vote_count >= 3:
                            # Mark this product class as being processed
            ...
            for track_id in list(pending_scans.keys()):
                ...
                # If the object reappeared in tracking
                if track_id in active_tracks:
                    # NEW: Track consecutive reappearances instead of immediately canceling
                    if track_id not in reappearance_tracking:
                        reappearance_tracking[track_id] = [frame_num, 1]
                        print(f"⚠️ [{timestamp}] Object briefly reappeared, monitoring... (ID: {track_id})")
                    else:
                        # Update consecutive frame count
                        if frame_num - reappearance_tracking[track_id][0] == 1:  # Consecutive frame
                            reappearance_tracking[track_id][0] = frame_num
                            reappearance_tracking[track_id][1] += 1
                            
                            # Check if we've reached the threshold to cancel the scan
                            if reappearance_tracking[track_id][1] >= reappearance_threshold:
                                pending_scans.pop(track_id)
                                # Clean up the products being processed
                                products_being_processed.pop(most_voted_cls, None)
                                reappearance_tracking.pop(track_id)
                                print(f"🔄 [{timestamp}] Object confirmed reappearance ({reappearance_threshold}+ frames), canceling scan (ID: {track_id})")
                        else:
                            # Non-consecutive frame, reset counter
                            reappearance_tracking[track_id] = [frame_num, 1]
                    continue
                else:
                    # Object disappeared again, remove from reappearance tracking if it was there
                    if track_id in reappearance_tracking:
                        reappearance_tracking.pop(track_id)
                        print(f"✓ [{timestamp}] Object disappeared again, continuing scan (ID: {track_id})")
