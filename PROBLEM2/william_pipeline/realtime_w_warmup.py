...
COOLDOWN = 0.5
WARMUP = 0.25  # Warmup period before scanning
class_id_to_name = {
...
def get_scan_zone(resolution):
    if resolution == 480:
        return (300, 150, 630, 550)
    elif resolution == 720:
        return (450, 220, 820, 740)
    elif resolution == 1080:
        return (700, 300, 1280, 1120)
    else:
...
def process_video(video_path, model, resolution, device='cpu'):
    ...
    # For warmup period before scanning - prevent premature classification
    first_detection_frame = {}  # track_id -> frame when first detected
    warmup_frames = int(WARMUP * fps)  # 0.3 seconds worth of frames
    ...
    while cap.isOpened():
        ...
        # Run detection on every frame for more consistent tracking of stationary objects
        ...
        # FIX: Move this code outside the detection loop
        ...
        for trk in tracked_objects:
            ...
            # Record first detection if this is a new track
            if track_id not in first_detection_frame:
                first_detection_frame[track_id] = frame_num
            ...
            if len(position_history[track_id]) >= 5:
                ...
                total_x_movement = abs(positions[-1][0] - positions[0][0])
                total_y_movement = abs(positions[-1][1] - positions[0][1])
                if total_x_movement < 15 and total_y_movement < 15:
                    is_stationary = True
                    # Highlight stationary objects
                    if is_inside_scan_area((x1, y1, x2, y2), scan_zone):
                        color = (0, 165, 255)  # Orange for stationary items in scan zone
                    else:
                        color = (255, 165, 0)  # Light blue for other stationary items
            ...
            if is_stationary:
                label += " [S]"  # Mark stationary objects
            ...
            if track_id not in scanned_log and is_inside_scan_area((x1, y1, x2, y2), scan_zone):
                ...
                # Check if object has been tracked long enough (warmup period)
                if can_scan and frame_num - first_detection_frame[track_id] < warmup_frames:
                    can_scan = False
                    cv2.putText(display_frame, "WARMUP", (x1, y1-30), 
        ...
        # Add stationary object handling - check for objects in scan zone
        for trk in tracked_objects:
            x1, y1, x2, y2, track_id = map(int, trk[:5])
            
            # Find class ID for this track
            cls_id = None
            for det in detections:
                if all(abs(det[i] - trk[i]) < 8 for i in range(4)):  # fuzzy bbox match
                    cls_id = int(det[5])
            ...
            # Identify potentially stationary objects
            is_stationary = False
            if track_id in position_history and len(position_history[track_id]) >= 5:
                ...
                # If movement below threshold, consider it stationary
                if total_x_movement < 15 and total_y_movement < 15:
                    is_stationary = True
                    
            # Handle stationary objects in scan zone differently
            if is_stationary and is_inside_scan_area((x1, y1, x2, y2), scan_zone) and track_id not in scanned_log:
                ...
                # Check warmup period for stationary objects too
                if can_scan and frame_num - first_detection_frame[track_id] < warmup_frames:
                    can_scan = False
                    cv2.putText(display_frame, "WARMUP", (x1, y1-30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)

                # For stationary objects, we need a different scanning criteria
                # For example, if they've been stationary in the scan zone for several frames
                stationary_frames = len([p for p in position_history[track_id] 
                                      if scan_zone[0] <= p[0] <= scan_zone[2] and 
                                         scan_zone[1] <= p[1] <= scan_zone[3]])
                
                if can_scan and stationary_frames >= 10:  # Stationary in scan zone for 10+ frames
                    product_name = class_id_to_name.get(cls_id, f"Product_{cls_id}")
                    scanned_log[track_id] = (product_name, timestamp)
                    last_class_scan_time[cls_id] = frame_num
                    output_df.append({"timestamp": timestamp, "product": product_name, "track_id": track_id})
                    print(f"🛒 [{timestamp}] Scanned stationary: {product_name} (ID: {track_id})")
                    cv2.putText(display_frame, "STATIONARY SCANNED", (x1, y1-30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
